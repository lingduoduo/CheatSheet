...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDsdHVfdSgVaDqKCI68l8x9V4T8eaCrkpdP7pausTQ4IZroibUqVZ20rw/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDefy14gyK7BcCLpEdwOKzTXcOStZOVy1mfY4AS462pbQ6iaOj5pqEoNQ/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDnmrlUBQiaRyufJdTZnSoDUYlBgPQWnbGzyJl9NB9hPUfZtPOlQM5Yibg/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDx0E1bKqKcauyxXb4trTYbYe2v6EOn6fExWbmwCOleUUvPiatlHiaLJJg/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDNHjwSKdibbHRmf7PrjuxDSSGmlJcw3TibY3dxFU9hm783e6bibg3jzpsw/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
在 `Linux` 中一个命令的去向可以有3个地方：终端、文件、作为另外一个命令的入参。![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTD1eSiaktDCexxrKQS2UGrRCLy7rcK7vJ2aEGbXW7Cr3v7YF4g0XIeSPQ/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)命令一般都是通过键盘输入，然后输出到终端、文件等地方，它的标准用语是 `stdin` 、 `stdout` 以及 `stderr` 。

- 标准输入 `stdin` ，终端接收键盘输入的命令，会产生两种输出；
- 标准输出 `stdout` ，终端输出的信息（不包含错误信息）；
- 标准错误输出 `stderr` ，终端输出的错误信息。

![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDncO7weX8F5ficdcN9leJ4zXLmpghaXN5wZQcgicJnQe1zo1oHT11ic5LA/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDLhNHEp9WqTyZQcqbibV8yAiarT3ErFAYg1lMMOwuyZnzURicuf3NGbv4A/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDSd6NpWcibwMUvWm25t2mLy5ckHLtz4KZbGDFK6rgv3g1q0IrS0gK54w/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
[root@lion ~]# w 06:31:53 up 25 days,  9:53,  1 user,  load average: 0.00, 0.01, 0.05USER     TTY      FROM             LOGIN@   IDLE   JCPU   PCPU WHAT
root     pts/0    118.31.243.53    05:56    1.00s  0.02s  0.00s w 06:31:53：表示当前时间
up 25 days, 9:53：表示系统已经正常运行了“25天9小时53分钟”1 user：表示一个用户
...
```
[root@lion ~]# ps
  PID TTY          TIME CMD 1793 pts/0    00:00:00 bash 4756 pts/0    00:00:00 ps  
...
```
top - 07:20:07 up 25 days, 10:41,  1 user,  load average: 0.30, 0.10, 0.07Tasks:  67 total,   1 running,  66 sleeping,   0 stopped,   0 zombie
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDgnWeQnSPINsVXl1gqdgtLZQC3HBKWQDwSNsniaS2B7ib7zMp92YuWIKw/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDH8FkLNWyPgLEUI27YTA0z1aUKmSgTJWvVhmqtZxNKibkwpIa94LXVtQ/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
...
1、ssh root@172.20.10.1:22 # 端口号可以省略不写，默认是22端口2、输入连接密码后就可以操作远端服务器了
```
...
用一张图表示三种模式如何切换：![Image](https://mmbiz.qpic.cn/mmbiz/9aPYe0E1fb12HhSLxYibr9w8y4GKAfhTDE2faicVK1YIhSu16FAo9aEgGbCIbhlYTZiaABdeciaNEwxJEU9W7ln6WA/640?wx_fmt=other&wxfrom=5&wx_lazy=1&wx_co=1)
